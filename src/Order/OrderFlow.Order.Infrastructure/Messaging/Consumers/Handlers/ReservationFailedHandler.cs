...
public class ReservationFailedHandler(OrderDbContext orderDbContext)
{
    public async Task HandleAsync(ReservationFailedEvent payload, CancellationToken cancellationToken)
    {
        ...
        order.OrderSagaState.Reserve(payload.EventId, false);
        order.Cancel();
