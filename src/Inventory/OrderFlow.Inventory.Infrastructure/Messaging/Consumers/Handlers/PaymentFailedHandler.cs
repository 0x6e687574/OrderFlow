...
public class PaymentFailedHandler(InventoryDbContext inventoryDbContext)
{
    public async Task HandleAsync(PaymentFailedEvent payload, CancellationToken cancellationToken)
    {
        ...
        var releasedQuantities = reservations.ToDictionary(
            r => r.Sku,
            r => r.Quantity);

        UpdateReservationStatuses(reservations);
        UpdateStockItemQuantities(stockItems, releasedQuantities);
    ...
    private static void UpdateStockItemQuantities(
        ...
        Dictionary<string, int> releasedQuantities)
    {
        foreach (var stockItem in stockItems)
        {
            stockItem.Cancel(releasedQuantities[stockItem.Sku]);
        }
    }
}